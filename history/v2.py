...
def proline(t_,o_,c_,p_):
    ...
    global orderhistory
    global orderchistory
    global orderthistory
    global ok
    global notok
    global start_
    global buy
    ...
    try :
        comment = orderchistory[-1].split("@")
        count = int(comment[0])
        ghozashte = int(comment[1])
        turn = int(comment[2])
    except:
        count = count
        ghozashte = ghozashte
    ...
    data.append(["u","buystop", round(c_+p_, 5),buy,str(count)+'@'+str(ghozashte)+'@'+str(turn)])
    if t_ != 0 :
        for j in o_:
            if o_[j]['open_price'] == c_ and len(data) == 1 :
                data.append(["m",o_[j]['type'], round(c_, 5),positionprice,str(count)+'@'+str(ghozashte)+'@'+str(turn)])
    data.append(["d","sellstop", round(c_ - p_, 5),sell,str(count)+'@'+str(ghozashte)+'@'+str(turn)])
                        ...
                        for k in range(0,len(data_)) : #add
                            ...
                            if u == 0 :
                                ...
                                if (data_[k][1] == "buystop") :
                                    ...
                                    self.dwx.open_order(symbol=symbol, magic= magicnumber,order_type='buystop',price=base, lots=data_[k][3],stop_loss=lower,take_profit=upper,comment= data_[k][4])
                                if (data_[k][1] == "sellstop") :
                                    magicnumber = magicnumber + 1
                                    self.dwx.open_order(symbol=symbol, magic= magicnumber,order_type='sellstop',price=base, lots=data_[k][3],stop_loss=upper,take_profit=lower,comment= data_[k][4])
    ...
    def on_historic_trades(self):
        ...
        for i in self.dwx.historic_trades:
            if self.dwx.historic_trades[i]["pnl"] > 0 :
                ...
                internal_list3.append(self.dwx.historic_trades[i]["type"])
                if self.dwx.historic_trades[i]["comment"].find("@") != -1 :
                    a = self.dwx.historic_trades[i]["comment"].replace("[sl]","")
                    a = a.replace("[tp]","")
                    internal_list2.append(a)
                else:
                    internal_list2.append("")

            if self.dwx.historic_trades[i]["pnl"] < 0 :
                internal_list1.append(0)
                internal_list3.append(self.dwx.historic_trades[i]["type"])
                if self.dwx.historic_trades[i]["comment"].find("@") != -1 :
                    a = self.dwx.historic_trades[i]["comment"].replace("[sl]","")
                    a = a.replace("[tp]","")
        ...
        for i in range (len(internal_list1)-1,-1,-1):
            orderhistory.append(internal_list1[i])
            orderchistory.append(internal_list2[i])
            orderthistory.append(internal_list3[i])
        print(orderhistory)
        print(orderchistory)
        print(orderthistory)
