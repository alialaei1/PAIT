...
def proline(t_,o_,c_,p_):
    ...
    for i in range(-1,-len(orderhistory),-1):
        if orderhistory[i] == 1 :
            break
        else:
            count = count + 1
    ...
    positionprice = zarib[count] * z   
    if count < 7 :
        notok = zarib[count+1] * z
        ok = zarib[0] * z
    elif count == 7 :
        ok = zarib[0] * z
        notok = -1
    ...
    if t_ != 0 :
        for j in o_:
            if (o_[j]['type'] == 'buy' or o_[j]['type'] == 'sell') :
                if int(j) >= ucode :
                    sag = 1
                    psag = o_[j]['open_price']
                    ucode = int(j)
    ...
    if buy != -1:
        if sag == 0:
            ...
            data.append(["u","buystop", round(psag+p_, 5),buy])

    if t_ != 0 :
        for j in o_:
            #if o_[j]['open_price'] == c_ and len(data) == 1 :
            if (sag == 1 and len(data) == 1 and int(j) == ucode):
                #data.append(["m",o_[j]['type'], round(c_, 5),positionprice])
                data.append(["m",o_[j]['type'], round(psag, 5),positionprice])
    if sell != -1:
        if sag == 0:
            data.append(["d","sellstop", round(c_ - p_, 5),sell])
        else:
            data.append(["d","sellstop", round(psag - p_, 5),sell])
                ...
                if now > self.last_open_time + timedelta(seconds=1):
                    ...
                    if now < process + timedelta(seconds=55) and now > process + timedelta(seconds=1) and nc != nm:
                        ...
                        for k in range(0,len(data__)) : #hazf
                            ...
                            if u == 0 :
                                print("close order")
                                self.dwx.close_order(ticket = data__[k][0])
